...
class IKSymbolicNSolve:
    """Implements an IK solver using SymPy's nsolve for direct numerical equation solving.

    This solver formulates IK as a system of equations and uses SymPy's numerical
    solver (nsolve) to find joint angles. This is different from IKSymbolic which
    uses gradient-based optimization.

    For square systems (n_joints == n_equations), uses sympy.nsolve directly.
    For under-determined systems (n_joints > n_equations), uses scipy.optimize.fsolve
    with the extra DOF left at their initial values.

    Note: This solver does not support nogo zones. Joint limits are enforced by
    clamping the solution post-hoc.
    """
    ...
    def __init__(
        ...
        cumulative_angle: sp.Expr = sp.Float(0)
        ...
        # Pre-compile numerical functions for forward kinematics
        self._fk_x_func = sp.lambdify(self.theta_symbols, self.end_effector_x, "numpy")
        self._fk_y_func = sp.lambdify(self.theta_symbols, self.end_effector_y, "numpy")
        self._fk_angle_func = sp.lambdify(
            self.theta_symbols, self.end_effector_angle, "numpy"
        )
    ...
    def __call__(
        ...
        if state.model != self.model:
            raise ValueError("State model does not match IKSymbolicNSolve model")
        ...
        equations = [
            self.end_effector_x - target_x,
            self.end_effector_y - target_y,
        ]

        # If angle constraint is specified, add it as an equation
        if desired.ee_angle is not None:
            # Use atan2(sin, cos) for proper angle wrapping
            angle_diff = self.end_effector_angle - desired.ee_angle
            angle_eq = sp.atan2(sp.sin(angle_diff), sp.cos(angle_diff))
            equations.append(angle_eq)

        n_equations = len(equations)

        # Initial guess from current state, with small perturbation to avoid singularities
        x0 = np.array(state.current.joint_angles, dtype=float)

        # Perturb near-zero angles to avoid singular Jacobian at fully extended config
        for i in range(len(x0)):
            if abs(x0[i]) < 0.01:
                x0[i] = 0.1  # Small non-zero angle

        joint_angles: tuple[float, ...]

        try:
            if n_equations == self.n_joints:
                # Square system: use nsolve directly
                solution = sp.nsolve(
                    equations,
                    self.theta_symbols,
                    list(x0),
                ...
                if isinstance(solution, list) and len(solution) > 0:
                    sol_dict = solution[0]
                    joint_angles = tuple(
                        float(sol_dict[theta]) for theta in self.theta_symbols
                    )
                else:
                    joint_angles = tuple(
                        float(solution[theta]) for theta in self.theta_symbols
                    )
            else:
                # Non-square system (under-determined): use scipy's minimize
                # to find solution that minimizes sum of squared residuals
                # while staying close to initial guess
                eq_funcs = [
                    sp.lambdify(self.theta_symbols, eq, "numpy") for eq in equations
                ]

                # Small regularization weight to prefer solutions near initial guess
                reg_weight = 1e-6

                def objective(thetas: np.ndarray) -> float:
                    residuals = np.array([f(*thetas) for f in eq_funcs])
                    deviation = thetas - x0
                    return float(
                        np.sum(residuals**2) + reg_weight * np.sum(deviation**2)
                    )

                result = minimize(objective, x0, method="BFGS")
                joint_angles = tuple(float(a) for a in result.x)

            # Clamp to joint limits if specified
            clamped_angles = []
            for angle, bound in zip(joint_angles, self.joint_bounds):
                if bound is not None:
                    angle = max(bound[0], min(bound[1], angle))
                clamped_angles.append(angle)
            joint_angles = tuple(clamped_angles)

        except Exception as e:
            # If solver fails, return current position unchanged
            print(f"nsolve failed: {e}")
            joint_angles = state.current.joint_angles
